...
# 
# A=f'dis_intensity_{dis_intensity_type}'
# B=f'life_g{start_g}_to_m{start_m}'
# C=f'dis_{dis_intensity_type}_DB_g{start_g}_to_g{end_g}'
# D=f'dis_{dis_intensity_type}_DS_g{start_g}_to_g{end_g}'
# E=f'dis_{dis_intensity_type}_DM_g{start_g}_to_g{end_g}'
# 
# df = df_YZ.copy()
# # There is no need to copy df_YZ every time. Just delet column C, D, E to keep it light before start of next round of creating disaster variable. 
# 
# df[B] = None
# df[B][(start_m <= df[month_history]) & (df[month_history] <= end_m) & (start_g <= df[age_in_month]) & (df[age_in_month] <= end_g)] = 1   # This should work faster than above line. Last line command is terrible idea, do not run loops in Python DataFrame by rows. 
# 
# 
# # Group by ID and column B, find maximum of column A values in rows 
# df[C] = df.groupby(id_vars + [B])[A].transform('max')
# 
# # Group by ID and column B, find share of rows with column A being 1 
# df[D] = df.groupby(id_vars + [B])[A].transform(lambda x: round((x == 1).mean(), 2))
#     
...
# 
# df = df.sort_values(by = id_vars+ [D], ascending=[True, True, True, True, False])
# df1 = df.drop_duplicates(subset=id_vars, keep='first')
# df1 = df1[id_vars + [D]]         
# df_store = pd.merge(df_store, df1, left_on=id_vars, right_on=id_vars, how='left')
# 
# df = df.sort_values(by = id_vars+ [C], ascending=[True, True, True, True, False])
# df1 = df.drop_duplicates(subset=id_vars, keep='first')
# df1 = df1[id_vars + [C]]         
# df_store = pd.merge(df_store, df1, left_on=id_vars, right_on=id_vars, how='left')
# 
# # df = df.drop([C, D, E], axis=1)
# df = df[id_vars + [A]]
# =============================================================================
...
def fn_dis_var_DB_DS_DM(df, id_vars, month_history, start_m, end_m, age_in_month, start_g, end_g, df_store, A, B, C, D, E):
    '''
    Description
    -----------
...
# age_in_month='age_month_mics6_to_history'
# month_history='month_mics6_to_history'
# summary = df[[age_in_month, month_history]].describe()

# ++++++++++++++ Dictionary to store all the periods we want to include. ++++++++++++++

dict_timing = {}

for key in range(1, 17):  # Adjust the range as needed
...
# my_dict[1]["start_month"]

# df_store = fn_dis_var_DM(df, id_vars, month_history='month_mics6_to_history', start_m=start_m, end_m=end_m, age_in_month='age_month_mics6_to_history', start_g=start_g, end_g=end_g, df_store=df_store, A=f'dis_intensity_{dis_intensity_type}', B=f'life_g{start_g}_to_m{start_m}', E=f'dis_{dis_intensity_type}_DM_g{start_g}_to_m{start_m}')
...
# df_store = fn_dis_var_DM(df, id_vars, month_history='month_mics6_to_history', start_month = start_month, end_month = end_month, age_in_month='age_month_mics6_to_history', start_age = start_age, end_age = end_age, df_store=df_store, A=f'dis_intensity_{dis_intensity_type}', B=f'life_m{start_month}_to_m{end_month}', E=f'dis_{dis_intensity_type}_DM_m{start_month}_to_m{end_month}')
