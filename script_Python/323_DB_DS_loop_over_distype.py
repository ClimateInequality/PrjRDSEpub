...
if bl_test is True: 
    ...
    dir_csv_file = f'{dir_data}/data_to_est/child_lifecycle_loc_date_alltype_n{it_sample}_{st_run_computer}_DB_DS_Add.csv'
    
else:
    df_YZ = pd.read_csv(f'{dir_data}/data_intermediate/YZ_child_lifecycle_skeleton.csv')
    dir_csv_file = f'{dir_data}/data_to_est/child_lifecycle_loc_date_alltype_full_{st_run_computer}_DB_DS_Add.csv'
...
def fn_dis_var_DB_DS(df, id_vars, month_history, start_month, end_month, age_in_month, start_age, end_age, df_store, A, B, C, D):
    '''
    Description
    -----------
...
for key_type in range(1, 5):
    ...
    for key in range(20, 21): 
        ...
        if key in range(1, 14): 
            ...
            df_store, df = fn_dis_var_DB_DS(df, id_vars, month_history = month_history, start_month = start_month, end_month = end_month, age_in_month = age_in_month, start_age = start_age, end_age = end_age, df_store=df_store, A=f'dis_intensity_{dis_intensity_type}', B=f'life_m{start_month}_to_m{end_month}', C=f'dis_{dis_intensity_type}_DB_m{start_month}to{end_month}', D=f'dis_{dis_intensity_type}_DS_m{start_month}to{end_month}')

        # ----- Naming rule is different for this type 
        if key in range(14, 19): 

            df_store, df = fn_dis_var_DB_DS(df, id_vars, month_history = month_history, start_month = start_month, end_month = end_month, age_in_month = age_in_month, start_age = start_age, end_age = end_age, df_store=df_store, A=f'dis_intensity_{dis_intensity_type}', B=f'life_g{start_age}_to_g{end_age}', C=f'dis_{dis_intensity_type}_DB_g{start_age}to{end_age}', D=f'dis_{dis_intensity_type}_DS_g{start_age}to{end_age}')

        # ----- Naming rule is different for this type 
        if key in range(19, 21): 

            df_store, df = fn_dis_var_DB_DS(df, id_vars, month_history = month_history, start_month = start_month, end_month = end_month, age_in_month = age_in_month, start_age = start_age, end_age = end_age, df_store=df_store, A=f'dis_intensity_{dis_intensity_type}', B=f'life_g{start_age}_to_m{start_month}', C=f'dis_{dis_intensity_type}_DB_g{start_age}_to_m{start_month}', D=f'dis_{dis_intensity_type}_DS_g{start_age}_to_m{start_month}')
